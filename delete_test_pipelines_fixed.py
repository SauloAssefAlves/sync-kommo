...
def list_pipelines(account):
    ...
    try:
        ...
        logger.info(f"📊 Conta {account.subdomain} ({account.account_role}) - {len(pipelines)} pipelines encontrados:")
...
def main():
    ...
    pipelines_to_delete = ["teste ", "teste", "teste 2", "teste 3"]
    ...
    with app.app_context():
        ...
        for account in accounts:
            logger.info(f"\n🔄 Processando conta: {account.subdomain} ({account.account_role})")
