...
class KommoAPIService:
    ...
    def update_pipeline_stage(self, pipeline_id_or_stage_id: int, stage_id_or_data, stage_data: Dict = None) -> Dict:
        """Atualiza um estágio existente - aceita 2 ou 3 parâmetros"""
        if stage_data is None:
            # Chamada com 2 parâmetros: stage_id, stage_data
            stage_id = pipeline_id_or_stage_id
            data = stage_id_or_data
            return self._make_request('PATCH', f'/leads/pipelines/statuses/{stage_id}', data=data)
        else:
            # Chamada com 3 parâmetros: pipeline_id, stage_id, stage_data
            pipeline_id = pipeline_id_or_stage_id
            stage_id = stage_id_or_data
...
class KommoSyncService:
            ...
            def process_pipeline(master_pipeline, results):
                ...
                try:
                    if pipeline_name in existing_pipelines:
                        ...
                        # ✨ USAR CORES DA MASTER EM VEZ DE CORES PADRÃO
                        logger.info(f"🎨 Sincronizando cores dos estágios da master para '{pipeline_name}'")
                        ...
                        for i, master_stage in enumerate(master_pipeline['stages']):
                            ...
                            # 🎨 USAR COR DA MASTER EM VEZ DE COR PADRÃO
                            master_color = master_stage.get('color', '#f2f3f4')  # Fallback para cinza se não tiver cor
                            stage_data['color'] = master_color
                            logger.info(f"🎨 Estágio '{master_stage['name']}' usando cor da master: {master_color}")
                            ...
                            if master_stage.get('type', 0) == 1:
                                ...
                                for created_stage in created_stages:
                                    if created_stage.get('type') == 1:
                                        master_stage_id = master_stage.get('master_id', master_stage.get('id'))
                                        if master_stage_id:
    ...
    def _sync_pipeline_stages(self, slave_api: KommoAPIService, master_pipeline: Dict, slave_pipeline_id: int, mappings: Dict):
        ...
        # Cores oficiais do Kommo (exatas da interface)
        kommo_colors = [
        ...
        def normalize_color_for_kommo(color):
            """Converte cores inválidas para cores válidas do Kommo"""
            # Converter para minúsculas para comparação
            color_lower = color.lower()
            
            # Lista exata das cores válidas do Kommo (da interface oficial)
            valid_colors = [
                '#fffeb2', '#fffd7f', '#fff000', '#ffeab2', '#ffdc7f', '#ffce5a',
                '#ffdbdb', '#ffc8c8', '#ff8f92', '#d6eaff', '#c1e0ff', '#98cbff',
                '#ebffb1', '#deff81', '#87f2c0', '#f9deff', '#f3beff', '#ccc8f9',
            ...
            # Mapeamento de cores inválidas para válidas mais próximas
            color_mappings = {
                '#99ccff': '#98cbff',  # azul claro inválido → azul claro válido
                '#ffff99': '#fffd7f',  # amarelo inválido → amarelo válido  
                '#ffcc66': '#ffce5a',  # laranja inválido → laranja válido
                '#ccff66': '#deff81',  # verde inválido → verde válido
                '#d5d8db': '#e6e8ea',  # cinza inválido → cinza válido
                '#c1c1c1': '#e6e8ea',  # cinza claro → cinza válido
            }
            
            # Se a cor está no mapeamento, usar a cor válida
            if color_lower in color_mappings:
                valid_color = color_mappings[color_lower]
                logger.info(f"🎨 Convertendo cor inválida {color} → {valid_color}")
                return valid_color
            
            # Se a cor já está na lista de cores válidas, usar como está
            if color_lower in [c.lower() for c in valid_colors]:
                return color_lower
            
            # Se não encontrou, usar uma cor padrão (cinza claro)
            logger.warning(f"⚠️ Cor {color} não reconhecida, usando cor padrão #f2f3f4")
            return '#f2f3f4'
        ...
        for i, master_stage in enumerate(master_pipeline['stages']):
            try:
                ...
                # 🎨 USAR COR DA MASTER NORMALIZADA PARA O KOMMO
                raw_master_color = master_stage.get('color', '#f2f3f4')  # Fallback para cinza se não tiver cor
                master_color = normalize_color_for_kommo(raw_master_color)
                stage_data['color'] = master_color
                logger.info(f"🎨 Estágio '{stage_name}' usando cor normalizada: {master_color}")
                ...
                if stage_exists:
                    # Estágio já existe - verificar se a cor precisa ser atualizada
                    existing_stage = existing_stages[stage_name]
                    slave_stage_id = existing_stage['id']
                    existing_color = existing_stage.get('color', '')
                    
                    if existing_color.lower() != master_color.lower():
                        logger.info(f"🎨 Atualizando cor do estágio '{stage_name}' de {existing_color} para {master_color}")
                        # Atualizar cor do estágio existente
                        try:
                            slave_api.update_pipeline_stage(slave_pipeline_id, slave_stage_id, {'color': master_color})
                            logger.info(f"✅ Cor do estágio '{stage_name}' atualizada com sucesso")
                        except Exception as color_error:
                            logger.warning(f"⚠️ Não foi possível atualizar cor do estágio '{stage_name}': {color_error}")
                    else:
                        logger.info(f"✅ Estágio '{stage_name}' já tem a cor correta: {existing_color}")
