"""
🔐 TESTE DE SINCRONIZAÇÃO DE ROLES (Com Flask App Context)
...
def test_sync_roles_with_flask():
    """Testa a sincronização de roles com contexto Flask"""
    print("🔐 TESTE DE SINCRONIZAÇÃO DE ROLES (COM FLASK)")
    print("=" * 60)
    
    # === INICIALIZAR FLASK APP ===
    print("⚙️ Inicializando contexto Flask...")
    
    try:
        from src.main import app
        with app.app_context():
            return _execute_sync_test()
    except Exception as e:
        print(f"❌ Erro ao inicializar Flask: {e}")
        return

def _execute_sync_test():
    """Executa o teste dentro do contexto Flask"""
    ...
    try:
        ...
        with open('sync_roles_test_flask_report.json', 'w', encoding='utf-8') as f:
            json.dump(report, f, indent=2, ensure_ascii=False)
        
        print(f"\n📄 Relatório salvo em: sync_roles_test_flask_report.json")
